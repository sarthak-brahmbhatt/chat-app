...
- **IAM: a scoped-down (not admin) IAM user**, created manually by the
  ...
  something generated by Codex. Its access keys are what later get
...
### 3.11 Clinical Note Extractor — Phase 1

A separate Python/FastAPI service lives in `clinical-extractor-service/`, but
stays in this repository so one Docker Compose stack can run the learning
project end to end. It is reached as another ordinary chat bot: Angular
keeps the existing WebSocket protocol, and chat-service relays the note to the
Python service and relays safe progress statuses plus the final JSON back.

- Each extraction is one-shot. It does not use `previous_response_id`.
- PHI is detected and redacted locally before any OpenAI request. Permitted PHI
  may be restored locally in the final response.
- Pydantic models define and validate the final structured output.
- Qdrant stores one point per ICD-10/CPT row and one point per quality measure,
  including source-document, source-section, version, and checksum metadata.
- Coding principles, tool contracts, general extraction rules, and the base job
  description are fixed instructions sent on every model request, not Qdrant
  search results.
- A programmatic, rerunnable DOCX ingestion command parses the supplied
  reference documents and idempotently update Qdrant.
- MySQL stores field-level evidence, confidence, tool calls, source references,
  and final output for audit. This is an explainable audit trace, not private
  model chain-of-thought.
- Phase 1 handles one note at a time. Batch processing, retries, and a
  dead-letter queue are deferred to the stretch-goal phase.

The runtime and ingestion sequences are documented in
[`docs/clinical-note-extractor-sequence.md`](docs/clinical-note-extractor-sequence.md).
...
- Bot chat flow (WebSocket, same envelope as any other chat — AGENTS.md 3.9):
...
13. Angular frontend, second pass — double tick's visual state was already bundled into
    ...
    server-enforced-expiry design — AGENTS.md 3.3), store the newly-issued access +
