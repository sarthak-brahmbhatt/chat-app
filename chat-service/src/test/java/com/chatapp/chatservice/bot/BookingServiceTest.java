...
class BookingServiceTest {
    ...
    void freeSlot_isBookedAndTheModelsConfirmationIsSent() {
        ...
        BookingOutcome outcome = bookingService.book(bookDecision(TOMORROW.toString()), USER_ID);
    ...
    void dateInThePast_isRejectedBeforeTheSlotIsEvenChecked() {
        ...
        BookingOutcome outcome = bookingService.book(bookDecision(YESTERDAY.toString()), USER_ID);
    ...
    void todayIsNotTreatedAsThePast() {
        ...
        assertThat(bookingService.book(bookDecision(TODAY.toString()), USER_ID).booked()).isTrue();
    }
    ...
    void bookWithNoAvailabilityId_isRejected() {
        ...
        BotDecision decision = new BotDecision(
                MODEL_CONFIRMATION, BotAction.BOOK, Optional.empty(), Optional.of(TOMORROW.toString()));

        assertThat(bookingService.book(decision, USER_ID).booked()).isFalse();
        verify(appointmentRepository, never()).saveAndFlush(any());
    }
    ...
    void unparseableDate_isRejected() {
        ...
        assertThat(bookingService.book(bookDecision("next Tuesday"), USER_ID).booked()).isFalse();
        verify(appointmentRepository, never()).saveAndFlush(any());
    }

    @Test
    void impossibleCalendarDate_isRejected() {
        assertThat(bookingService.book(bookDecision("2026-02-30"), USER_ID).booked()).isFalse();
        verify(appointmentRepository, never()).saveAndFlush(any());
    }
    ...
    private BotDecision bookDecision(String date) {
        return new BotDecision(
                MODEL_CONFIRMATION, BotAction.BOOK, Optional.of(AVAILABILITY_ID), Optional.of(date));
    }
