...
public class BookingService {
     ...
     *
     * @param decision the model's turn, which must be a booking request
     ...
     */
    @Transactional
    public BookingOutcome book(BotDecision decision, long userId) {
        Optional<Long> availabilityId = decision.availabilityId();
        Optional<LocalDate> date = parseDate(decision.bookedForDate());
        ...
        if (availabilityId.isEmpty() || date.isEmpty()) {
            log.warn("Model asked to BOOK without a usable availability_id/booked_for_date: {}", decision);
        ...
        try {
            ...
            return BookingOutcome.booked(decision.replyToUser(), saved.getId());
