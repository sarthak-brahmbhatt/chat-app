...
public class DoctorAssistantBotService {
    ...
    public BotReply handleUserMessage(String userId, String botId, String messageId, String content, Instant sentAt) {
        ...
        if (turn.decision().isBookingRequest()) {
            // Translated into the clinic's own vocabulary rather than passed
            // through — BookingService must not know what a BotDecision is, or
            // the tool-calling bot could not reuse it.
            BookingRequest bookingRequest = new BookingRequest(
                    turn.decision().availabilityId().orElse(null),
                    turn.decision().bookedForDate().orElse(null),
                    turn.decision().replyToUser());
            BookingOutcome outcome = bookingService.book(bookingRequest, Long.parseLong(userId));
